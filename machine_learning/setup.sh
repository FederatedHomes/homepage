...
create_certificate_placeholders() {
  ...
  local starter_host="${STARTER_SUPERLINK_HOST:-fl.example.internal}"
  mkdir -p "$tls_dir"
  ...
  if [ ! -f "$ca_crt" ] || { [ "$role" = "server" ] && [ ! -f "$superlink_crt" ]; }; then
...
EOF
    ...
    chmod 600 "$ca_key"
    [ -f "$superlink_key" ] && chmod 600 "$superlink_key"
  fi
...
STARTER FEDERATION TLS CERTIFICATES

These files contain valid STARTER certificates generated by setup.sh so the
federated clients can establish an initial TLS connection during development
and integration testing.

They are NOT the federation's final production PKI credentials. Replace them
with certificates issued by the federation's production CA/PKI when available.

Starter SuperLink hostname:
  $starter_host

Required on the SERVER host:
  ca.crt         - Starter/federation CA certificate
  superlink.crt  - SuperLink server certificate
  superlink.key  - SuperLink server private key

Required on each CLIENT host:
  ca.crt         - Same CA certificate that signed superlink.crt

IMPORTANT:
  - The starter SuperLink certificate is valid only when SUPERLINK_ADDRESS
    resolves to a hostname/IP covered by its SAN (by default $starter_host,
    localhost, or 127.0.0.1).
  - If the server is reached using a different LAN DNS name or IP, replace
    the starter SuperLink certificate with one whose SAN matches that address.
  - Do not copy superlink.key to client hosts.
  - Do not commit real federation certificates or private keys to source control.
  - The starter CA signing key is generated temporarily and deleted by setup.sh.
EOF

  echo "Starter TLS material is ready in $tls_dir"
